...
<!-- メイン -->
    <div class="mt-40 h-full text-center">
        <form class="mb-6 mt-10" action="{{ route('res.presentation') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mx-auto max-w-2xl mt-10  px-4  sm:px-6 lg:max-w-7xl lg:px-8">
                <label class="w-full h-full block flex justify-center mb-10">
                    <textarea class="px-4 py-4 resize-none w-full sm:w-full md:w-1/2 h-70 md:h-64 block rounded-md shadow-md focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="message" id="" cols="30" rows="10" placeholder="返信を書いてください" type="text" style="font-size:20px">{{ old('message') }}</textarea>
                </label>
                <form class="mt-10">
